            ...
            <div className="main">
                <div className="dashboard-content pt-3"> 
                    ...
                    <section className="py-4">
                        <div className="title-socials-logo d-flex justify-content-between align-items-center flex-wrap">
                            <div className="title-socials">
                                <h3 className="pb-3" style={{ textTransform: 'capitalize' }}>{ brand?.data?.name }</h3> 
                                <div className="socials d-flex align-items-center gap-3 flex-wrap">
                                    { brand?.data?.web_address && 
                                        <span className="d-flex align-items-center gap-1">
                                            ...
                                            <span>{ brand?.data?.web_address }</span>
                                        </span> }
                                    { brand?.data?.facebook && 
                                        <span className="d-flex align-items-center gap-1">
                                            <span>
                                    ...
                                    { brand?.data?.instagram && 
                                        <span className="d-flex align-items-center gap-1">
                                            ...
                                            <span>{ brand?.data?.instagram }</span>
                                        </span> }
                                    { brand?.data?.twitter_x && 
                                        <span className="d-flex align-items-center gap-1">
                                            ...
                                            <span>{ brand?.data?.twitter_x }</span>
                                        </span> }
                                    { brand?.data?.other_social && 
                                        <span className="d-flex align-items-center gap-1">
                                            ...
                                            <span style={{ textTransform: 'capitalize' }}>{ brand?.data?.other_social_handle }</span>
                                        </span> }
                                </div>
                            </div> 
                    ...
                    <section className="products">
                        <h3 className="border-bottom pb-1 fs-5 w-100">Products associated with&nbsp;<span style={{ textTransform: 'capitalize' }}>{ brand?.data?.name }</span></h3>

                        <section>
                            { (brand?.data?.products?.length > 0) ? 
                                <>
                                    <PaginationMeter 
                                        current_page={ brand?.data?.products?.meta?.current_page } 
                                        limit={ brand?.data?.products?.meta?.limit } 
                                        total_pages={ brand?.data?.products?.meta?.total_pages } 
                                        total_results={ brand?.data?.products?.meta?.total_results } />

                                    <ul>
                                        { (brand?.data?.products?.length > 0) && brand?.data?.products?.map((product, index) => {
                                            return ( 
                                                <li 
                                                    key={ product?._id } 
                                                    className="box-shadow-1 border-radius-25 py-3 px-2 cursor-pointer">
                                                        <ProductComponent1 
                                                            index={ (((brand?.data?.products?.meta?.current_page) > 1) 
                                                                        ? (((brand?.data?.products?.meta?.current_page - 1) * brand?.data?.products?.meta?.limit) + 1) 
                                                                            : brand?.data?.products?.meta?.current_page) + index } 
                                                            itemId={ product?._id } 
                                    ...
                                    : (
                                        <div className="h-100 w-100 py-4 d-flex justify-content-center align-items-center">
                                            <span>There are no products associated with the&nbsp;<span style={{ textTransform: 'capitalize' }}>{ brand?.data?.name }</span>&nbsp;brand.</span>
                                        </div>
                                    ) } 
