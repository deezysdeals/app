        ...
        <Layout>
            <div className="main">
                <div className="dashboard-content pt-3"> 
                    <h2 className="border-bottom pb-1 fs-4">Sold Products from Sales</h2> 
                    ...
                    <section className="py-4">
                        <ul className="list-unstyled d-flex flex-column gap-5">
                            { (sales?.data?.length > 0) ? (sales?.data?.map((product, index) => {
                                return ( 
                                    <li 
                                        key={ product?._id } 
                                ...
                                <div className="d-flex justify-content-center align-items-center p-5">No sales yet</div>
                            ) }
                        </ul>
                    </section>
                </div>
                ...
                { (sales?.data?.length > 0) &&
                    <section className="pagination-links py-5 d-flex justify-content-end gap-2 pe-2"> 
                        ...
                        <span 
                            ...
                            onClick={ async () => { 
                                scrollToTop(); 
                                await getSales((sales?.meta?.current_page >= 1) ? (sales?.meta?.current_page - 1) : 1); 
                            } }>
                                <Previous /> 
                        </span> 
                        <span 
                            ...
                            onClick={ async () => { 
                                scrollToTop(); 
                                await getSales((sales?.meta?.current_page < sales?.meta?.total_pages) ? (sales?.meta?.current_page + 1) : sales?.meta?.total_pages); 
                            } }>
                            <Next /> 
                        </span> 
                        <span 
                            ...
                            onClick={ async () => { 
                                scrollToTop(); 
                                await getSales(sales?.meta?.total_pages); 
                            } }>
                                <Last />
                        </span>
                    </section> 
